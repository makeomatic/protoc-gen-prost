...
* `package_separator=[-_.+]`: Changes the character used to separate parts
  of a package name when used as a feature flag. Defaults to `-` if not
  specified. Note: `.` results in feature names that are not legal for
